#!/usr/bin/env bash
# Stop all running AI-song synthesis jobs and release their worker resources.
set -euo pipefail
...
for job in jobs:
    if job.get("kind") == "synthesize" and job.get("status") == "running":
        print(job.get("id"))
PY
)

if [ "${#JOB_IDS[@]}" -eq 0 ]; then
  echo "没有正在合成 AI 乐曲的任务。"
  exit 0
fi

echo "准备停止 ${#JOB_IDS[@]} 个 AI 乐曲合成任务..."
for job_id in "${JOB_IDS[@]}"; do
  echo "→ cancel synthesize job $job_id"
